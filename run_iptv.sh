...
# 用于向 GitHub Actions 传递数据的临时文件
GITHUB_OUTPUT_FILE="iptv_outputs.txt"
: > "$GITHUB_OUTPUT_FILE"
...
fetch_and_merge() {
  ...
  while [ $attempt -lt $max_attempts ]; do
    ...
    # 失败重试逻辑
    echo "   ⚠️ Attempt $attempt failed (Exit Code: $curl_exit_code)"
...
# 7. 写入初步统计数据到文件
echo "success_count=$success_count" >> "$GITHUB_OUTPUT_FILE"
echo "failed_count=$failed_count" >> "$GITHUB_OUTPUT_FILE"
echo "total_raw_channels=$total_fetched" >> "$GITHUB_OUTPUT_FILE"
...
# 10. 更新最终统计
echo "valid_channels=$valid_channels" >> "$GITHUB_OUTPUT_FILE"
echo "invalid_channels=$invalid_channels" >> "$GITHUB_OUTPUT_FILE"
