...
SOURCE_PRIORITY[YueChan]=30
SOURCE_PRIORITY[myIPTV]=40
SOURCE_PRIORITY[Default]=99

convert_m3u_to_txt() {
  awk 'BEGIN{channel=""}
       /^#EXTINF/{n=split($0,a,",");channel=a[n];gsub(/^[ \t]+|[ \t]+$/,"",channel);next}
       /^http/{if(channel!=""){print channel","$0;channel=""}else{print $0}}'
}
...
fetch_and_merge() {
  ...
  while [ $attempt -lt $max_attempts ]; do
    ...
    raw_content=$(curl -sL --connect-timeout 15 --max-time 30 -k --retry 2 --retry-delay 1 -H "User-Agent: Mozilla/5.0" "$url" 2>&1)
    ...
    if [ $? -eq 0 ] && [ -n "$raw_content" ]; then
      if [[ ! "$raw_content" =~ ^<.*>$ ]]; then
        ...
        echo "$name - OK" >> success_sources.txt
        return 0
      fi
    fi
...
if [ -s live_deduped.txt ]; then
  while IFS= read -r line; do
    ...
    if [[ ! "$url" =~ ^https?:// ]]; then
      ...
      status=$(curl -I -s -o /dev/null -w "%{http_code}" --connect-timeout 3 --max-time 5 -k "$url" 2>/dev/null)
...
echo "invalid_channels=$invalid_channels" >> iptv_outputs.txt
...
grep -vi "CCTV" live_valid.txt | grep -vi "卫视" > Regional.txt 2>/dev/null || : > Regional.txt

echo "IPTV Script Finished"
exit 0
